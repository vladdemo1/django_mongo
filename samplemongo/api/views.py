...
def api_overview(request):
    ...
    api_urls = {
        'List': '/databases-list/',
        'Database Detail': '/database-detail/<str:database>/',
        ...
        'Delete from Collection': '/delete-element/<str:database>/<str:collection>/<str:mode>/?param=value',
    }
...
@api_view(['GET'])
def database_detail(request, database):
    """
...
def collection_detail_filter(request, database, collection):
    ...
    """
    client = get_client()
    options = request.GET.items()
    data = client[database][collection].find(dict(options))
...
def parse_json(data):
    """
    Parse current data for json var
    """
